...
public class AggiornamentoInterfaccia {
	...
	public void aggiornaNomeGiocatori(){
		...
		for(int i=0; i<aggiornamento.getGiocatori().size(); i++){
			...
			if(giocatoreAgg.getNome().equals(frame.getNomeGiocatore()) && giocatoreAgg.getColore().equals(frame.getColore())){
				frame.getPlancia().setNomeGiocatore(frame.getNomeGiocatore(), frame.getColore());
			}
			else{
				frame.getPlanciaAvversario().getPlance().get(planceRiempite).setNomeGiocatore(aggiornamento.getGiocatori().get(i).getNome(), aggiornamento.getGiocatori().get(i).getColore());
				planceRiempite++;
			}
		}
	}
		...
		for(int i=0; i<aggiornamento.getGiocatori().size(); i++){
			for(int j=0; j<aggiornamento.getGiocatori().get(i).getScomuniche().length; j++){
				if(aggiornamento.getGiocatori().get(i).getScomuniche()[j]){
					frame.getTabellone().getCarteScomunica().get(j).add(new CuboScomunica(aggiornamento.getGiocatori().get(i).getColore()));
				}
			}
		}
	}
			...
			if(giocatoreAgg.getNome().equals(frame.getNomeGiocatore()) && giocatoreAgg.getColore().equals(frame.getColore())){
				...
				for(int j=0; j<4; j++){
					if(giocatoreAgg.getFamigliariPlancia().get(j) != null){
						...
						String giocatoreAppartenenza = giocatoreAgg.getFamigliariPlancia().get(j).getGiocatoreAppartenenza();
						famigliariPlancia[j] = new Famigliare(valore, numero, colore, giocatoreAppartenenza);
					}
					else famigliariPlancia[j] = null;
	...
	public void aggiornaTurno(){
		...
		frame.getTabellone().getLblTurnoPeriodo().setText("<html><br>TURNO: "+frame.getTurno()+"</br><br>PERIODO: "+(frame.getTurno()+1)/2+"</br></html>");
	}
