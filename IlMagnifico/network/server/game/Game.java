 ...
 */
/* package-local */ class Game extends Partita {
	...
	// private static final int ASSISTANTS_TO_PAY_TO_MAKE_ADDITIONAL_MAIN_ACTION
	// = 3;
	...
	// * @return an update state to send to all clients.
	// */
	// /* package-local */ UpdateState electCouncillor(Player player, Councilor
	// councillor, String region,
	// boolean addCoins) {
	...
	// */
	// /* package-local */ UpdateState acquireBusinessPermitTile(Player player,
	// GameTurn turn,
	// List<PoliticCard> politicCards, String region, int index) throws
	// LogicException {
	// List<PoliticCard> usedPoliticCards = satisfyBalcony(region,
	// politicCards);
	...
	// BusinessPermitTile takenBusinessPermitTile =
	// takeBusinessPermitTile(region, index);
	...
	// updateState.changeBusinessPermitTileAtIndex(getBusinessPermitTile(region,
	// index), region, index);
	...
	// * emporium.
	// */
	// /* package-local */ UpdateState buildEmporium(Player player, GameTurn
	// turn, BusinessPermitTile businessPermitTile,
	// String cityName) throws LogicException {
	// City city = getCity(cityName);
	// businessPermitTile.validateCity(city);
	// int assistants =
	// city.getAssistantCountToBuildEmporium(player.getNickname());
	...
	// */
	// /* package-local */ UpdateState buildEmporium(Player player, GameTurn
	// turn, List<PoliticCard> politicCards,
	// List<String> cities) throws LogicException {
	// List<PoliticCard> usedPoliticCards =
	// satisfyBalcony(AbstractBoard.KING_REGION, politicCards);
	// int coinsToPay = getCoinsToPayToSatisfyBalcony(usedPoliticCards);
	...
	//
	// /**
	// * Collect all rewards related to all city directly connected with
	// provided
	...
	// return getLinkedCities(city).stream().filter(linkedCity ->
	// linkedCity.hasPlayerAnEmporium(player))
	// .map(City::getReward).collect(Collectors.toCollection(ArrayList::new));
	// }
	//
	// /**
	// * After building an emporium this method check if the player has built
	// all
	...
	// * @param player
	// * that has built an emporium
	// * @return {@link #VICTORY_POINTS_FOR_LAST_EMPORIUM} if player has built
	// all
	...
	// * where the player has built the emporium.
	// */
	// private void checkTypeBonus(Map<String, Reward> rewards, Player player,
	// City city) {
	...
	// * where the player has built the emporium.
	// */
	// private void checkRegionBonus(Map<String, Reward> rewards, Player player,
	// City city) {
	...
	// }
	// }
	// rewards.put(region.getRegionType(),
	// mBonusCards.get(region.getRegionType()));
	// }
	//
	// /**
	// * Apply all bonus earned while building an emporium to the player. It
	// will
	// * add a reward king card for every bonus card earned. If there are no
	// king
	...
	// * to update.
	// */
	// private void applyBonus(Player player, City city, UpdateState
	// updateState) {
	...
	// */
	// private void applyRewards(Player player, GameTurn turn, List<Reward>
	// rewards, UpdateState updateState) {
	// int assistants = 0;
	// int victoryPoints = 0;
	// int nobilityStep = 0;
	// int coins = 0;
	...
	// new Reward(assistants, victoryPoints, nobilityStep, coins, politicCards,
	// mainActions));
	// }
	//
	// /**
	// * Apply a single reward to a player. Also the updateState will be
	// updated.
	// *
	// * @param player
	// * that is earning the reward.
	...
	// */
	// private void applyReward(Player player, GameTurn turn, UpdateState
	// updateState, Reward reward) {
	...
	// mKingBoard.getNobilityTrackReward(player.getNobilityTrackPosition());
	...
	// */
	// private int getCoinsToPayToMoveKing(List<String> cities) throws
	// RouteNotValid {
	// try {
	...
	//
	// /**
	// * Validate two cities: it will check if the who cities are directly
	// linked.
	// *
	// * @param city1
	// * to check.
	...
	// */
	// private void validateCities(String city1, String city2) throws
	// RouteNotValid {
	// if (city2 != null && !areCitiesLinked(city1, city2)) {
	// throw new RouteNotValid(String.format("%s is not linked to %s", city1,
	// city2));
	// }
	// }
	//
	...
	// */
	// /* package-local */ UpdateState engageAssistant(Player player) throws
	// LogicException {
	...
	// */
	// /* package-local */ UpdateState changeBusinessPermitTiles(Player player,
	// String region) throws NotEnoughAssistants {
	...
	// updateState.changeBusinessPermitTileAtIndex(regionBoard.getFirstVisibleTile(),
	// region,
	// RegionBoard.INDEX_FIRST_BPT);
	// updateState.changeBusinessPermitTileAtIndex(regionBoard.getSecondVisibleTile(),
	// region,
	...
	// * if player as no enough assistant to make the action.
	// */
	// /* package-local */ UpdateState sendAssistantToElectCouncillor(Player
	// player, Councilor councillor, String region)
	// throws NotEnoughAssistants {
	// player.payAssistants(ASSISTANTS_TO_SEND_TO_ELECT_COUNCILLOR);
	// UpdateState updateState = electCouncillor(player, councillor, region,
	// false);
	...
	// * if player as no enough assistant to make the action.
	// */
	// /* package-local */ UpdateState performAdditionalMainAction(Player
	// player) throws NotEnoughAssistants {
	...
	//
	// /**
	// * Apply all bonus of all players and create a list of update states to
	// send
	// * to clients.
	// *
	...
	// return updateStates.values().toArray(new
	// UpdateState[updateStates.size()]);
	// }
	//
	// /**
	// * On the end of the game, all bonus cards and all king rewards owned by
	// the
	...
	// */
	// private void applyBonusAndKingRewards(Player player, UpdateState
	// updateState) {
	// int victoryPoints = 0;
	...
	//
	// /**
	// * On the end of the game: "the player furthest ahead on the nobility
	// track
	// * earns 5 victory points, and the player in the second place earn 2
	// victory
	...
	// * be updated.
	// */
	// private void applyNobilityTrackPositionRewards(Map<String, UpdateState>
	// updateStates) {
	// List<Player> players = new ArrayList<>(mPlayers.values());
	// Collections.sort(players, (player1,
	// player2) -> Integer.compare(player1.getNobilityTrackPosition(),
	// player2.getNobilityTrackPosition())
	// * -1);
	...
	// * be updated.
	// */
	// private void applyMostPermitTileBonus(Map<String, UpdateState>
	// updateStates) {
	// List<Player> players = new ArrayList<>(mPlayers.values());
	// Collections.sort(players, (player1, player2) ->
	// Integer.compare(player1.getBusinessPermitTiles().size(),
	...
	// List<Player> players = new ArrayList<>(mPlayers.values());
	// Collections.sort(players, (player1, player2) -> {
	// int comparator = Integer.compare(player1.getVictoryPoints(),
	// player2.getVictoryPoints()) * -1;
	// if (comparator == 0) {
	// comparator = Integer.compare(player1.getAssistants() +
	// player1.getPoliticCards().size(),
	// player2.getAssistants() + player2.getPoliticCards().size()) * -1;
	// }
	// return comparator;
	// });
	// return
	// players.stream().map(Player::getNickname).collect(Collectors.toList());
	// }
	//
	// /**
	...
	// * if none of the given politic cards can satisfy this balcony.
	// */
	// private List<PoliticCard> satisfyBalcony(String region, List<PoliticCard>
	// politicCards)
	// throws BalconyUnsatisfiable {
	...
	// */
	// private BusinessPermitTile takeBusinessPermitTile(String region, int
	// index) {
	// RegionBoard regionBoard = (RegionBoard) getAbstractBoard(region);
	// switch (index) {
	// case 1:
	...
	// * if index is not 1 nor 2.
	// */
	// private BusinessPermitTile getBusinessPermitTile(String region, int
	// index) {
	// RegionBoard regionBoard = (RegionBoard) getAbstractBoard(region);
	// switch (index) {
	// case 1:
	...
	// */
	// /* package-local */ UpdateState earnFirstSpecialRewards(Player player,
	// GameTurn turn, List<String> cities)
	...
	// !city.hasPlayerAnEmporium(player)) {
	...
	// * @return the update state bundle that contains all items changed.
	// */
	// /* package-local */ UpdateState earnSecondSpecialRewards(Player player,
	// GameTurn turn, List<String> regions,
	// List<Integer> indices) {
	// List<BusinessPermitTile> businessPermitTiles = new ArrayList<>();
	// for (int i = 0; i < regions.size(); i++) {
	// businessPermitTiles.add(takeBusinessPermitTile(regions.get(i),
	// indices.get(i)));
	// }
	...
	// getBusinessPermitTile(regions.get(i), indices.get(i));
	// updateState.changeBusinessPermitTileAtIndex(businessPermitTile,
	// regions.get(i), indices.get(i));
	// }
	// return updateState;
	// }
	//
	...
	// * @return the update state bundle that contains all items changed.
	// */
	// /* package-local */ UpdateState earnThirdSpecialRewards(Player player,
	// GameTurn turn,
	// List<BusinessPermitTile> businessPermitTiles) {
	// UpdateState updateState = new UpdateState(player.getNickname());
	// businessPermitTiles.stream()
	// .filter(businessPermitTile ->
	// player.getBusinessPermitTiles().contains(businessPermitTile)
	// || player.getUsedBusinessPermitTiles().contains(businessPermitTile))
	// .forEach(businessPermitTile -> applyReward(player, turn, updateState,
	// businessPermitTile));
	// return updateState;
	// }
	//
	// /**
	// * Internal builder class. This is the only way to instantiate a
	// ServerGame
	// * instance.
	// */
	...
	// * @return the builder instance.
	// */
	// /* package-local */ Builder setCenterRegion(RegionBoard
	// centerRegionBoard) {
	...
	// * @return the builder instance.
	// */
	// /* package-local */ Builder setPoliticCards(List<PoliticCard>
	// politicCards) {
	...
	// setBusinessPermitTiles(List<BusinessPermitTile> businessPermitTiles) {
	...
	// * @return the builder instance.
	// */
	// /* package-local */ Builder setNobilityTrack(Map<Integer, NobilityReward>
	// nobilityTrack) {
	...
	// * @return the builder instance.
	// */
	// /* package-local */ Builder setRewardKingCards(List<Reward>
	// rewardKingCards) {
	...
	// int regionTiles = (int) Math.ceil((double) mBusinessPermitTiles.size() /
	// 3);
	// ServerGame serverGame = new ServerGame();
	// serverGame.mLeftRegionBoard = mLeftRegionBoard;
	// serverGame.mLeftRegionBoard.fillBalcony(mCouncilors);
	// serverGame.mLeftRegionBoard.addBusinessPermitTiles(mBusinessPermitTiles.subList(0,
	// regionTiles));
	...
	// .addBusinessPermitTiles(mBusinessPermitTiles.subList(regionTiles, 2 *
	// regionTiles));
	...
	// .addBusinessPermitTiles(mBusinessPermitTiles.subList(2 * regionTiles,
	// mBusinessPermitTiles.size()));
	...
	// */
	// private void setupPlayer(int position, Player player, ServerGame
	// serverGame) {
